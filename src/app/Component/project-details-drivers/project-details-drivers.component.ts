...
export class ProjectDetailsDriversComponent implements OnInit {
  ...
  addDrivers() {
    this.driverData = {
      businessDrivers: {
        driverName: [
          ...
          this.others_BD
            ? this.addDriversForm.get("others_BD_Text").value
            : this.addDriversForm.get("others_BD_Text").setValue(""),
        ],
      },
      itDrivers: {
        driverName: [
          ...
          this.Automation,
          this.others_IT
            ? this.addDriversForm.get("Others_IT_Text").value
            : this.addDriversForm.get("Others_IT_Text").setValue(""),
        ],
      },
    };
    ...
    this.projectService
      .addDriverstobackend(this.projectId, this.driverData)
      .subscribe(
        (data) => {
          this._notificationToast.showSuccess("Driver Details Added Successfully","Driver Added Successfully")
          this.router.navigate(["overview"]);
        },
        (err) => {
          this._notificationToast.showError("Cannot Add Drivers",`${err.error.errorMessage}`)
          console.log(err);
        }
      );
  }
