...
export class ProjectDetailsScopeComponent implements OnInit {
  ...
  addScopes() {
    ...
    this.projectService
      .addScopestobackend(this.projectId, this.scopeData)
      .subscribe(
        (data) => {
          console.log(data);
          this._notificationToast.showSuccess("Scope Details Added Successfully","Scope Added Successfully")
          this.router.navigate(["overview"]);
        },
        (err) => {
          this._notificationToast.showError("Cannot Add Scopes",`${err.error.errorMessage}`)
          console.log(err);
        }
      );
  }
